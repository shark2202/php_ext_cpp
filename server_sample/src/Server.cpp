...
PHP_METHOD(simple_server, __construct)
{
    ...
    server_set_object(server_object, tcpEventServer);//保存到全局的变量中 obj就是php中的对象的id,
}
...
{
    zval* server_object = getThis();
    TcpEventServer *tcpEventServer = (TcpEventServer *)server_get_object(server_object);//从全局变量中获取当前对象对应的扩展的server对象
...
static void simple_server_onreceive(ServerCbParam *param){
    ...
    //这个就是执行一个方法的，匿名方法都可以执行的
    //cb是方法的名称
    //call_user_function_ex(function_table, object, function_name, retval_ptr, param_count, params, no_separation, symbol_table)
